...
锁消除：不可能有别人跟我竞争锁，则消除锁。

## Atomic***

使用了CAS乐观锁实现相比于锁性能更高。更新的时候会比较旧值是否与期望值相等，相等则更新旧值为新值，否则不更新。

## Lock与Condition

Condition将Object的wait，notify拆解出来成一个类。

Lock与synchronized相比更灵活，并且同一个锁可以有多个条件，可以实现读写分离，可以查看是否获取锁，可以方便实现更加灵活的优先级锁

ReentrantLock可重入锁，即同一个类可以lock多次，但必须有对应数量的unlock，一个对象可以重复持有一个锁。
