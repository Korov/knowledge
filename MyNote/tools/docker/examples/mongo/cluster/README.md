...
# 启动分片
docker run --name mongo_shard1 -d --network=mongo_net --hostname=mongo_shard1 -v $PWD/db1:/data/db mongo:5.0.3 mongod --port 27017 --shardsvr --replSet mongo_shard1
docker run --name mongo_shard2 -d --network=mongo_net --hostname=mongo_shard2 -v $PWD/db2:/data/db mongo:5.0.3 mongod --port 27017 --shardsvr --replSet mongo_shard2
...
# 进入docker
docker exec -it mongo_shard1 bash
# 打开mongo连接
mongosh
# 执行以下语句
rs.initiate({_id: "mongo_shard1", version: 1, members: [ { _id: 0, host : "mongo_shard1:27017" } ] })

docker exec -it mongo_shard2 bash
# 打开mongo连接
mongosh
# 执行以下语句
rs.initiate({_id: "mongo_shard2", version: 1, members: [ { _id: 0, host : "mongo_shard2:27017" } ] })
...
mongosh
# 执行以下语句
sh.addShard("mongo_shard1/mongo_shard1:27017")
sh.addShard("mongo_shard2/mongo_shard2:27017")
