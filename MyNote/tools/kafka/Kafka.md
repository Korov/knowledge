...
**auto.offset.reset**：指定了消费者在读取一个没有偏移量（offset）的分区或者偏移量无效的情况下（因消费者长时间失效，包含偏移量的记录已经过时井被删除）该作何处理，默认值是 `latest`。earliest指定从最早的位移开始消费，这里最早的位移不一定是0；latest：指定从最新位移处开始消费；none：指定如果未发现位移信息或位移越界，则抛出异常。
